...
namespace FRELODYAPIs.Controllers
{
    ...
    public class PlaylistsController : ControllerBase
    {
        ...
        [Authorize]
        [ProducesResponseType(typeof(bool), 200)]
        public async Task<IActionResult> UpdatePlaylistSongTranspose([FromQuery] string playlistId, [FromQuery] string songId, [FromQuery] int transpose)
        {
            var result = await _playlistService.UpdatePlaylistSongTransposeAsync(playlistId, songId, transpose);
            if (!result.IsSuccess)
                return StatusCode(result.StatusCode, result.Error?.Message ?? "Error");
            return Ok(result.Data);
        }
        ...
        [Authorize]
        [ProducesResponseType(typeof(bool), 200)]
        public async Task<IActionResult> UpdatePlaylistGlobalTranspose([FromQuery] string playlistId, [FromQuery] int globalTranspose)
        {
            var result = await _playlistService.UpdatePlaylistGlobalTransposeAsync(playlistId, globalTranspose);
            if (!result.IsSuccess)
                return StatusCode(result.StatusCode, result.Error?.Message ?? "Error");
            return Ok(result.Data);
        }
